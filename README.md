...
Ideas arrive faster than they can be organized, so the two `#`-prefixed folders are staging areas:
raw material lands in `#incoming/`, and once its substance has been folded into a topic note the
original is archived in `#log/`. Nothing is deleted, so where an idea came from stays traceable.
